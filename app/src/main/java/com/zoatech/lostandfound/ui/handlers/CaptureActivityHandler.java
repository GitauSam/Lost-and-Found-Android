package com.zoatech.lostandfound.ui.handlers;
...
import com.zoatech.lostandfound.utils.DecodeThread;
    ...
    public void handleMessage(Message message) {
        ...
        switch (message.what) {
            ...
            case R.id.ocr_decode_failed:
                state = State.PREVIEW;
                activity.setShutterButtonClickable(true);
                Toast toast = Toast.makeText(activity.getBaseContext(), "Document capture failed. Please try again.", Toast.LENGTH_SHORT);
...
//    CameraManager.get().stopPreview();
    ...
    void clearAutoFocusListener() {
        activity.clearAutoFocusListener();
    }

    void setupAutoFocusListener() {
        activity.setupAutoFocusListener();
    }

    /**
     * Request OCR when the on-screen shutter button is clicked.
     */
    public void requestOCRDecode() {
        if (!isDecoding) {
